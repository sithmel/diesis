...
const dependencyMemo = diesis.dependencyMemo
  ...
  describe("4 functions", () => {
    ...
    it("must throw on invalid dependencies", async () => {
      ...
      try {
        ...
        assert.equal(
          e.message,
          "A dependency should depend on an array of dependencies or values (not an array)"
        )
      }
    })
  ...
  describe("4 functions with memo", () => {
    ...
    it("must not memoize when function throw", async () => {
      ...
      try {
        ...
        assert.equal(buggy._dep.isMemoized, false)
      }
    })
